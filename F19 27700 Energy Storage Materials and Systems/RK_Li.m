...
data_graph = load('LiRuSn.txt');
% (a)
figure(1); hold off;
plot(data_graph(:,1),data_graph(:,2),'.');
title('Voltage Against State of Charge')
xlabel('State of Charge (x)')
ylabel('Voltage at State of Charge U(x) (V)')
grid on;

% (b)
xmaxLi = max(data_graph(:,1))
% data_graph(:,1) = data_graph(:,1)/xmaxLi;
% Above line leads to error in fit, normalize by 4 to avoid x1 = 1;
data_graph(:,1) = data_graph(:,1)/4;
figure(2); hold off
plot(data_graph(:,1),data_graph(:,2),'.');
title('Voltage Against State of Charge')
xlabel('filling fraction (x1)')
ylabel('Voltage at State of Charge U(x) (V)')
grid on;
